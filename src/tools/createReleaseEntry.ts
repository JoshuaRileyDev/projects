...
export default async function createReleaseEntry(input: Input) {
    ...
    if (releases.length === 0) {
        ...
        const newRelease = await createRelease({ projectID: input.projectID, releaseType: "patch", version: projectSetting?.initialVersion || "1.0.0" });
    ...
    const newReleaseEntry = { ...input, id: allReleaseEntries.length + 1, releaseID: latestRelease.id, type: input.entryType };
